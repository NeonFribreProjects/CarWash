...
export const sendBookingConfirmation = async ({
  ...
  const customerMailOptions = {
    ...
    to: customerEmail,
    subject: 'Booking Confirmation - Mobile Car Wash',
    html: `
...
export const sendBookingStatusUpdate = async ({
  ...
  const customerMailOptions = {
    from: process.env.EMAIL_USER,
    to: customerEmail,
    subject: `Booking ${status.charAt(0).toUpperCase() + status.slice(1)} - Mobile Car Wash`,
    html: status === 'confirmed' ? `
      <h1>Booking Confirmed</h1>
      <p>Dear ${customerName},</p>
      <p>Your car wash booking has been confirmed and payment of 20 CAD has been received:</p>
      <ul>
        <li>Service: ${serviceName}</li>
        <li>Date: ${startTime.toFormat('EEEE, MMMM d, yyyy')}</li>
        <li>Time: ${startTime.toFormat('h:mm a')} - ${endTime.toFormat('h:mm a')}</li>
    ...
    ` : `
      ...
      <p>Dear ${customerName},</p>
      <p>Your car wash booking has been cancelled:</p>
      <ul>
        <li>Service: ${serviceName}</li>
        <li>Date: ${startTime.toFormat('EEEE, MMMM d, yyyy')}</li>
        <li>Time: ${startTime.toFormat('h:mm a')} - ${endTime.toFormat('h:mm a')}</li>
